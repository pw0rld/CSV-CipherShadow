...
class SimStateStorageMixin(VEXMixin):
    ...
    def _perform_vex_expr_Get(self, offset, ty, action=None, inspect=True):
        value = self.state.registers.load(offset, self._ty_to_bytes(ty), action=action, inspect=inspect)

        reg_addr = self.reg_index[self.state.solver.eval(offset)][0]
        self.state.reg_value_trace[value] = reg_addr
        self.state.value_trace[value] = (reg_addr, 'dir')


        return value

    def _perform_vex_expr_RdTmp(self, tmp):
        # The vex ir tmps from this structure.
        value = self.state.scratch.tmp_expr(tmp)
        
        if tmp not in self.state.reg_value_trace or tmp not in self.state.value_trace:
            raise Exception(f"[Error]SimState _perform_vex_expr_RdTmp tmp: {tmp} not in reg_value_trace or value_trace.")
        else:
            if value not in self.state.reg_value_trace:
                self.state.reg_value_trace[value] = self.state.reg_value_trace[tmp]
            if value not in self.state.value_trace:
                self.state.value_trace[value] = self.state.value_trace[tmp]
            

        return value

    def _perform_vex_expr_Load(self, addr, ty, endness, action=None, inspect=True, condition=None, **kwargs):
        value = self.state.memory.load(
            addr, self._ty_to_bytes(ty), endness=endness, action=action, inspect=inspect, condition=condition
        )
        if addr in self.state.reg_value_trace:
            self.state.reg_value_trace[value] = self.state.reg_value_trace[addr]
            self.state.reg_var_trace[value] = self.state.reg_value_trace[addr]
            self.state.reg_var_trace[addr] = self.state.reg_value_trace[addr]
        else:
            self.state.reg_value_trace[value] = addr

        if addr in self.state.value_trace:
            self.state.var_trace[addr] = self.state.value_trace[addr]

        self.state.pointer_trace[addr] = (value, 'r')
        self.state.value_trace[value] = (addr, 'r')
        self.state.var_trace[value] = (addr, 'r')

        if addr not in self.state.trace_store[self.state.imark_addr]:
            self.state.trace_store[self.state.imark_addr][addr] = value
    ...
    def _perform_vex_stmt_Put(self, offset, data, action=None, inspect=True):
        reg_addr = self.reg_index[self.state.solver.eval(offset)][0]
        if data not in self.state.reg_value_trace:
            self.state.reg_value_trace[reg_addr] = data
            self.state.reg_var_trace[reg_addr] = data
        else:
            self.state.reg_value_trace[reg_addr] = self.state.reg_value_trace[data]
            self.state.reg_var_trace[reg_addr] = self.state.reg_value_trace[data]

        if data not in self.state.value_trace:
            self.state.value_trace[reg_addr] = (data, 'dir')
            self.state.var_trace[reg_addr] = (data, 'dir')
        else:
            self.state.value_trace[reg_addr] = self.state.value_trace[data]
            self.state.var_trace[reg_addr] = self.state.value_trace[data]

        self.state.trace_store[self.state.imark_addr][reg_addr] = data
    ...
    def _perform_vex_stmt_Store(self, addr, data, endness, action=None, inspect=True, condition=None):
        if (
            ...
            if addr not in self.state.reg_value_trace:
                raise Exception(f"[Error]_perform_vex_stmt_Store addr is symbolic: {addr} not in reg_value_trace.")
            else:
                self.state.reg_var_trace[addr] = self.state.reg_value_trace[addr]
                self.state.trace_store[self.state.imark_addr][addr] = concrete_data

            if addr not in self.state.value_trace:
                raise Exception(f"[Error]_perform_vex_stmt_Store addr is symbolic: {addr} not in value_trace.")
            else:
                self.state.var_trace[addr] = self.state.value_trace[addr]
                self.state.trace_store[self.state.imark_addr][addr] = concrete_data

            if data not in self.state.reg_value_trace:
                self.state.reg_value_trace[addr] = concrete_data
            else:
                self.state.reg_value_trace[addr] = self.state.reg_value_trace[data]

            if data not in self.state.value_trace:
                self.state.value_trace[addr] = (concrete_data, 'dir')
            else:
                self.state.value_trace[addr] = self.state.value_trace[data]

        if data not in self.state.reg_value_trace:
            self.state.reg_value_trace[addr] = data
        else:
            self.state.reg_value_trace[addr] = self.state.reg_value_trace[data]

        if data not in self.state.value_trace:
            self.state.value_trace[addr] = (data, 'dir')
        else:
            self.state.value_trace[addr] = self.state.value_trace[data]

        if addr not in self.state.reg_value_trace:
            raise Exception(f"[Error]SimState _perform_vex_stmt_Store addr is symbolic: {addr} not in reg_value_trace.")
        else:
            self.state.reg_var_trace[addr] = self.state.reg_value_trace[addr]
            self.state.trace_store[self.state.imark_addr][addr] = data

        if addr not in self.state.value_trace:
            raise Exception(f"[Error]SimState _perform_vex_stmt_Store addr is symbolic: {addr} not in value_trace.")
        else:
            self.state.var_trace[addr] = self.state.value_trace[addr]
            self.state.trace_store[self.state.imark_addr][addr] = data

        self.state.pointer_trace[addr] = (data, 'w')
    ...
    def _perform_vex_stmt_WrTmp(self, tmp, data, deps=None):
        if tmp in self.state.reg_value_trace or tmp in self.state.value_trace:
            raise Exception(f"[Error]_perform_vex_stmt_WrTmp tmp: {tmp} already in reg_value_trace or value_trace.")
        else:
            if data not in self.state.reg_value_trace:
                raise Exception(f"[Error]_perform_vex_stmt_WrTmp data: {data} not in reg_value_trace.")
            else:
                self.state.reg_value_trace[tmp] = self.state.reg_value_trace[data]
                self.state.value_trace[tmp] = self.state.value_trace[data]
...
class HeavyVEXMixin(SuccessorsMixin, ClaripyDataMixin, SimStateStorageMixin, VEXMixin, VEXLifter):
    ...
    def _handle_vex_stmt_IMark(self, stmt):
        ...
        self.state.imark_pointer_trace.append(self.state.pointer_trace)
        self.state.pointer_trace = dict()
        self.state.imark_reg_var_trace.append(self.state.reg_var_trace)
        self.state.reg_var_trace = dict()
        self.state.imark_var_trace.append(self.state.var_trace)
        self.state.var_trace = dict()
        self.state.imark_addr = hex(ins_addr)
        # self.state.trace_store[self.state.imark_addr] = []
        self.state.trace_store[self.state.imark_addr] = dict()
    ...
    def _perform_vex_stmt_Exit(self, guard, target, jumpkind):
        ...
        if o.COPY_STATES not in self.state.options:
            ...
            exit_state = self.state.copy()
            exit_state.registers.trace_store = self.state.registers.trace_store
            exit_state.memory.trace_store = self.state.memory.trace_store
            exit_state.imark_pointer_trace = self.state.imark_pointer_trace
            exit_state.imark_reg_var_trace = self.state.imark_reg_var_trace
            exit_state.imark_var_trace = self.state.imark_var_trace
            exit_state.reg_value_trace = self.state.reg_value_trace
            exit_state.value_trace = self.state.value_trace
            exit_state.pointer_trace = self.state.pointer_trace
            exit_state.reg_var_trace = self.state.reg_var_trace
            exit_state.var_trace = self.state.var_trace
            exit_state.trace_store = self.state.trace_store
            exit_state.imark_addr = self.state.imark_addr
            cont_state = self.state
    ...
    def _perform_vex_stmt_Dirty_call(self, func_name, ty, args, func=None):
        ...
        for arg in args:
            if isinstance(arg, pyvex.expr.RdTmp):
                arg = arg.tmp
            if arg in self.state.reg_value_trace:
                if isinstance(self.state.reg_value_trace[arg], list):
                    reg_sources += self.state.reg_value_trace[arg]
                else:
                    reg_sources.append(self.state.reg_value_trace[arg])

            if arg in self.state.value_trace:
                if isinstance(self.state.value_trace[arg], list):
                    source += self.state.value_trace[arg]
                else:
                    source.append(self.state.value_trace[arg])

        self.state.reg_value_trace[retval] = reg_sources
        self.state.value_trace[retval] = source
        return retval
    ...
    def _perform_vex_expr_Load(self, addr, ty, endness, **kwargs):
        value = super()._perform_vex_expr_Load(addr, ty, endness, **kwargs)
    ...
    def _perform_vex_expr_CCall(self, func_name, ty, args, func=None):
        ...
        value = super()._perform_vex_expr_CCall(func_name, ty, args, func=None)
        ...
        self.state.reg_value_trace[value] = reg_sources
        self.state.value_trace[value] = source
        return value
