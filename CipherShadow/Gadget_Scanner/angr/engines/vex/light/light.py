...
class VEXMixin(SimEngineBase):
    ...
    def _perform_vex_expr_RdTmp(self, tmp):
        ...
        if tmp not in self.state.reg_value_trace:
            raise Exception(f"[Error]light _perform_vex_expr_RdTmp tmp: {tmp} not in reg_value_trace.")
        elif value not in self.state.reg_value_trace:
            self.state.reg_value_trace[value] = self.state.reg_value_trace[tmp]
            
        if tmp not in self.state.value_trace:
            raise Exception(f"[Error]light _perform_vex_expr_RdTmp tmp: {tmp} not in value_trace.")
        elif value not in self.state.value_trace:
    ...
    def _handle_vex_expr_Op(self, expr):
        value = self._perform_vex_expr_Op(expr.op, [self._handle_vex_expr(arg) for arg in expr.args])
        reg_sources = []
        source = []
        ...
        self.state.reg_value_trace[value[0]] = reg_sources
        self.state.value_trace[value[0]] = source
    ...
    def _perform_vex_stmt_WrTmp(self, tmp, data):
        if tmp in self.state.reg_value_trace or tmp in self.state.value_trace:
            raise Exception(f"[Error]light _perform_vex_stmt_WrTmp tmp: {tmp} already in reg_value_trace or value_trace.")
        else:
            if data not in self.state.reg_value_trace:
                self.state.reg_value_trace[tmp] = data
            else:
                self.state.reg_value_trace[tmp] = self.state.reg_value_trace[data]
                
            if data not in self.state.value_trace:
                self.state.value_trace[tmp] = (data, 'dir')
            else:
    ...
    def _analyze_vex_defaultexit(self, *a, **kw):
        value = self._handle_vex_expr(*a, **kw)
        ...
        del self.state.imark_pointer_trace[0]
        del self.state.imark_reg_var_trace[0]
        del self.state.imark_var_trace[0]
        return value
    ...
    def handle_vex_block(self, irsb: pyvex.IRSB):
        ...
        for reg, value in self.state.arch.registers.items():
            if value[0] not in self.reg_index or self.reg_index[value[0]][1] < value[1]:
                self.reg_index[value[0]] = (reg, value[1])
