...
Execute the `locate_page()` function in `analysis_csv.py`. After completion, you will obtain an `md5.txt` file. Search for the virtual address corresponding to `79bdda39f48c484c47751869c735b2a6` in this file. In our example:
...
## Gadget Scanner
### Description
A tool for searching potentially exploitable gadgets for the CipherShadow attack. Implement data flow tracing based on Angr to extract data flow information against functions.
...
### Run
You can obtain control flow and data flow gadgets by executing the following command:
```bash
python gadget_search.py <Binary of Target Application> <Target Function> <Json File of Initial State>
```
...
```bash
cd One_Shot_Ciphertext_Leakage
