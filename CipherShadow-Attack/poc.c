...
#define KVM_TLB_POSITION  _IOWR(KVMIO, 0x1a, struct kvm_tlb_position)
struct kvm_tlb_position {
	__u64 src_addr;
};
...
int tlb_position(int fd, uint64_t src_addr) {
    struct kvm_tlb_position param;
    param.src_addr = src_addr;
    int ret = ioctl(fd, KVM_TLB_POSITION, &param);
    printf("result: %d\n", ret);
    return ret;
}
...
int main(int argc, char const *argv[])
{
    ...
    if (argc < 2) {
        printf("Usage: %s <vc81 | copy16 dst_gpa src_gpa>\n", argv[0]);
        open_kvm_vc_81(fd);
        return 1;
    }


    if (strcmp(argv[1], "getfeature") == 0) {
        if (argc != 3) {
            printf("Usage: %s getfeature gpa\n", argv[0]);
            return 1;
        }
        ...
        if (argc != 6) {
            printf("Usage: %s copy16 <src_gpa> <dst_gpa> <dst_gpa_1> <dst_gpa_2>\n", argv[0]);
            return 1;
        }
    ...
    else if (strcmp(argv[1], "tlbposition") == 0) {
        if (argc != 3) {
            printf("Usage: %s tlbposition src_addr\n", argv[0]);
            return 1;
        }
        uint64_t src_addr = strtoull(argv[2], NULL, 0);
        tlb_position(fd, src_addr);
    }
    else if (strcmp(argv[1], "single_step_page") == 0) {
        if (argc != 4) {
            printf("Usage: %s single_step_page dst_gpa single_pic_interval\n", argv[0]);
            return 1;
        }
